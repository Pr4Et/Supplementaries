...
public:
    ...
    // New lightweight constructor : build a 1 - frame decoder from preloaded bytes
    ElectronCountedFramesDecompressor(const EerFrameSettings & settings, const std::vector<unsigned char>&frameBytes);

    // Accessors to support preload-once / decode-later pipeline
    const EerFrameSettings& frameSettings() const;
    bool isTiffMode() const;
    size_t getPreloadedFrameCount() const;

    // Copy compressed bytes for frame i into a vector (works for TIFF and ECC modes)
    std::vector<unsigned char> copyFrameData(size_t i) const;
