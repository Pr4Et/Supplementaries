...
// Keep existing static LUT, but ensure it's initialized once (thread-safe)
//static float BSplineLUT[4][4];
static std::once_flag g_bspline_once;
...
//Added by Shahar
// ---- Lightweight single-frame ctor for thread-per-bin decode ----
ElectronCountedFramesDecompressor::ElectronCountedFramesDecompressor(
    const EerFrameSettings& settings,
    const std::vector<unsigned char>& frameBytes)
    : m_nElectronsCounted(0)
{
    // We will use the TIFF-mode path that reads from m_frameBuffers.
    // (This keeps BitStreamer construction identical to TIFF mode.)
    m_tiffMode = true;
    m_eerFrameSettings = settings;

    m_frameBuffers.clear();
    m_frameBuffers.push_back(frameBytes); // single frame
    m_nFrames = 1;
    m_fsizeBytes = frameBytes.size();
    m_frameCounter = 0;

    // Thread-safe LUT init
    static std::once_flag g_bspline_once;
    std::call_once(g_bspline_once, createBSplineLUT);
}

// ---- Accessors ----
const EerFrameSettings& ElectronCountedFramesDecompressor::frameSettings() const
{
    return m_eerFrameSettings;
}

bool ElectronCountedFramesDecompressor::isTiffMode() const
{
    return m_tiffMode;
}

size_t ElectronCountedFramesDecompressor::getPreloadedFrameCount() const
{
    return static_cast<size_t>(m_nFrames);
}

// Copy compressed bytes for frame i (TIFF or ECC) into std::vector<unsigned char>
std::vector<unsigned char> ElectronCountedFramesDecompressor::copyFrameData(size_t i) const
{
    if (m_tiffMode)
    {
        if (i >= m_frameBuffers.size())
            throw std::out_of_range("copyFrameData: frame index out of range (TIFF mode).");
        return m_frameBuffers[i]; // already a vector<unsigned char>
    }
    else
    {
        // ECC mode: slice from m_globalBuffer using m_frameStartPointers
        if (i + 1 >= m_frameStartPointers.size())
            throw std::out_of_range("copyFrameData: frame index out of range (ECC mode).");

        const uint64_t start = m_frameStartPointers[i];
        const uint64_t stop = m_frameStartPointers[i + 1];
        if (stop < start || stop > m_fsizeBytes)
            throw std::runtime_error("copyFrameData: invalid frame boundaries (ECC).");

        const size_t nBytes = static_cast<size_t>(stop - start);

        // m_globalBuffer is BitStreamWordType[]; convert to bytes
        const uint8_t* base = reinterpret_cast<const uint8_t*>(m_globalBuffer.data());
        const uint8_t* src = base + start;
	...
	: m_nElectronsCounted(0)
{
    ...
    // createBSplineLUT();  // <-- removed this direct call
    static std::once_flag g_bspline_once;
    std::call_once(g_bspline_once, createBSplineLUT);
}
