...
dx_numbers_values=input('Synchronization step? ');
...
parfor caseno=1:4 %parfor !!  %shift numbers
    shift_step_camera=dx_numbers_values;
    ...
    if use_hdf5==1
        newFilename=strrep(Chosen_Filename_file1,'data_000001.h5',sprintf('tiles_case%g.mrc',caseno));
    else
        newFilename=strrep(Chosen_Filename_file1,'.raw',sprintf('_tiles_case%g.mrc',caseno));
    end
...
for fileno=1:4 
    ...
    if use_hdf5==1
        Grand_Chosen_Filename_file1{fileno}=strrep(Chosen_Filename_file1,'data_000001.h5',sprintf('tiles_case%g.mrc',fileno));
    else
        Grand_Chosen_Filename_file1{fileno}=strrep(Chosen_Filename_file1,'.raw',sprintf('_tiles_case%g.mrc',fileno));
    end
end
