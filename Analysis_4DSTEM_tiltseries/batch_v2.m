% Simple batch to reconstruct BP-PCA and SIRT-piDPC1 from hdf5 tilt series
% Assuming data acquired using Arina camera with SavvyScan 4D-STEM software/system. No need for HAADF.
% Prerequirements:
% Decoding software for hdf5 files +bitshuffle compression filter (https://www.hdfgroup.org/downloads/)
% MatTomo (PEET project: https://bio3d.colorado.edu/imod/matlab.html), use the frozen/modified https://github.com/Pr4Et/Supplementaries/tree/main/Analysis_4DSTEM_tiltseries/MatTomo.
...
% found in this repository https://github.com/Pr4Et/Supplementaries/tree/main/Analysis_4DSTEM_tiltseries.
% Written by Shahar Seifer, Elbaum lab, Weizmann Institute of Science, (C)2024-6
...
function_rings_align(Chosen_Filename_ring1_reorder,tiltfile);
function_sect_iDPC(Chosen_Filename_sect1_reorder_preali);

filename=strrep(Chosen_Filename_ring1_reorder_preali,'_ring1_','_piDPC1_');
...
clusteralign_astra_reconstruct_BP(0,90,0,filename,tiltfile,'',1,300); %use BP instead for low resources or replace 1->2 for binning of 2

%Optionally generate PCA tomograms
flag_PCA=input('Continue to PCA? 1-yes, 0-no: ');
if flag_PCAC
	for ringno=1:16
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Align all tilt series based on one image as reference  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function function_rings_align(Chosen_Filename_ring1_reorder,tiltfile)
    ...
    Chosen_Filename=Chosen_Filename_ring1_reorder;
    ...
    for n=1:no_of_files
        ...
        for ind=1:veclength
            if mod(ind,movingavg_halfsize)==0
                ...
                %im(im>10*medv)=medv;
                meanv=mean(im(:));
                mask=im>0.5*meanv; %exclude noise
