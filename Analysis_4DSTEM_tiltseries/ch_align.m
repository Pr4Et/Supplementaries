...
[filename,path] = uigetfile('Z:\shared\SavvyscanData\*.mrc','Fetch good contrast tilt series (MRC file)');
Chosen_Filename=[path filename];

firstchannel=input('First channel number? ');
if firstchannel==1
[filename,path] = uigetfile('Z:\shared\SavvyscanData\CH1*.mrc','Fetch CH1 tilt series to align (MRC file)');
Chosen_Filename1=[path filename];
else
Chosen_Filename1=Chosen_Filename;    
end
...
if skip_cor==0
    ...
    if false
        ...
        for ind=1:ntilts
            ...
            r_temp=r_mn(view_in,canvas,600,1);
            if ~isnan(r_temp)
                r=r_temp;
            end
...
for channel=firstchannel:7
    nextfilename=strrep(Chosen_Filename1,'CH1',sprintf('CH%d',channel));
    mRCImage=MRCImage;%Instentiate MRCImage in mRCImage
...
function r_mn=r_mn(Imagem,Imagen,shift_limit,do_filt)
    if do_filt==1
        Imagem=imgaussfilt(Imagem-imgaussfilt(Imagem,100),7);
        Imagen=imgaussfilt(Imagen-imgaussfilt(Imagen,100),7);
    elseif do_filt==2
